...
# process cell metadata if provided ----------------------------------------------------------------

# load cell metadata file if provided and only
if (!is.null(snakemake@input$cell_metadata)) {
  message("Processing cell metadata...")
  cell_metadata <- fread(snakemake@input$cell_metadata)
} else {
  cell_metadata <- NULL
}
...
sce <- create_pert_sce(expr, annot = gene_tss, pert_status = pert_status,
                       grna_annot = guide_targets, cell_metadata = cell_metadata,
                       targets_pert_name = "cre_perts")
