...
simulate_diff_expr_pert_real <- function(pert, sce, pert_level, cell_batches, pert_input_function,
  ...
  ### START ADDED LINES
  # remove genes that have Na dispersion estimates, and therefore appear as NaN when counts are simulated
  # Get the row names from pert_object
  row_names_pert_object <- rownames(pert_object)
  # Filter out rows where the corresponding dispersion in sce is Na
  filtered_row_names <- row_names_pert_object[!is.na(rowData(sce)[row_names_pert_object, "dispersion"])]
  # Subset pert_object to keep only the filtered row names
  pert_object <- pert_object[filtered_row_names, ]
  ### END ADDED LINES
