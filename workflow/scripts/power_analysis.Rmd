...
```{r}
save.image("power_analysis_markdown.rda")
#stop()
```
...
# Sometimes there are multiple bins with low power, so we add a tiny bit of noise so that there are unique bins
set.seed(123) # for reproducibility
power <- power %>%
  mutate(cells_quant = cut(cells, breaks = quantile(cells, na.rm = TRUE), include.lowest = TRUE)) %>%
  mutate(avg_expr_jittered = avg_expr + runif(n = n(), min = -1e-6, max = 1e-6),
